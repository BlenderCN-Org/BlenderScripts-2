...
def copyVertexGroups(source_obj, dest_obj, vgroup_names):
    ...
    for name in vgroup_names:
        if name in dest_obj.vertex_groups:
            continue
...
def createShapeKeyMap(object):
    ...
    for i in range(len(object.data.shape_keys.key_blocks)):
        ...
        object.data.shape_keys.key_blocks[i].value = 0
    return name_to_id
...
def createVertexGroupShapeKeys(object, shape_key_map, source_name, upper):
    ...
    new_key_name = source_name + suffix
    if new_key_name in shape_key_map:
        return
    
    key = object.data.shape_keys.key_blocks[shape_key_map[source_name]]
    key.value = 1.0
    new_key = object.shape_key_add(name=str(new_key_name), from_mix=True)
...
def createUpperLowerShapeKeys(object, shape_key_map):
    ...
    createVertexGroupShapeKeys(object, shape_key_map, 'Expressions_mouth10_min', True)
...
def createSingleVisemeShapeKey(object, shape_key_map, mix, viseme_name):
    if viseme_name in shape_key_map:
        return
...
def createVisemeShapeKeys(object, shape_key_map):
    ...
    createSingleVisemeShapeKey(object, shape_key_map, {'Expressions_mouth03_min': 0.4,
        'Expressions_mouth05_max': 0.45, 'Expressions_mouth06_max': 0.45, 'Expressions_mouth07_min': 1.0,
        'Expressions_mouth09_min': 0.7, 'Expressions_mouth11_min': 0.7, 'Expressions_mouth12_min': 0.6},
        'Viseme_UW_UH_OY')
        
    createSingleVisemeShapeKey(object, shape_key_map, {'Expressions_mouth02_min': 0.55,
        'Expressions_mouth10_min': 0.75, 'Expressions_mouth09_min_upper': 0.3}, 'Viseme_AW')
        
    createSingleVisemeShapeKey(object, shape_key_map, {'Expressions_mouth09_min_upper': 0.35,
        'Expressions_mouth09_max_lower': 0.7}, 'Viseme_F_V')
...
class VisemesOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        scene = context.scene
        # TODO: ask for a topology, and the name accordingly
